...
create table if not exists public.social_plan_side_group_message_reactions (
  id uuid primary key default gen_random_uuid(),
  ...
  emoji text not null,
  created_at timestamptz not null default now(),
  constraint social_plan_side_group_message_reactions_emoji_check check (emoji in ('❤️', '😂', '🔥', '👏', '😮', '😢'))
);

create unique index if not exists social_plan_side_group_message_reactions_once_idx
  on public.social_plan_side_group_message_reactions(message_id, user_id);

create index if not exists social_plan_side_group_message_reactions_message_idx
  on public.social_plan_side_group_message_reactions(message_id, created_at desc);
...
alter table public.social_plan_side_group_message_reactions enable row level security;
